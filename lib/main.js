...
function iterator( mu, sigma, options ) {
	...
	if ( !isNumber( mu ) || isnan( mu ) ) {
		throw new TypeError( format( '0q36v,NI', mu ) );
	}
	if ( !isPositive( sigma ) ) {
		throw new TypeError( format( '0q372,Go', sigma ) );
	}
	if ( arguments.length > 2 ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0q32V,FD', options ) );
		}
		opts = assign( {}, options );
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0q32t,G9', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
